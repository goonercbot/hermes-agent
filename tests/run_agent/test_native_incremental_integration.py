...
def test_native_stream_progress_survives_host_idle_and_continues(tmp_path, monkeypatch, in_place):
    ...
    # Seed the prior fixture response through the v0.21.1 usage interface;
    # unanchored rough estimates intentionally defer until a real response.
...
def test_native_full_repair_keeps_unsealed_tool_pair_past_stale_cursor():
    ...
    # Exercise that real callable, not the obsolete AST-extracted branch.
    from agent.turn_iteration_prep import prepare_iteration
    from tests.run_agent.test_native_iteration_port import _iteration_agent
    iteration_agent = _iteration_agent()
    iteration_agent._last_flushed_db_idx = stale_cursor
    full = prepare_iteration(
        iteration_agent, messages=protected, api_call_count=1
    ).messages
    assert full == source_before
...
def test_empty_fallback_projects_valid_checkpoint_tail_for_minimax_chat(monkeypatch):
    ...
    monkeypatch.setattr('agent.turn_empty_response._empty_guard.empty_retry_budget',lambda *a,**k:1)
    try:
