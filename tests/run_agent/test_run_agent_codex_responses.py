...
def test_codex_final_preflight_blocks_middleware_split_tool_pair(monkeypatch, shape, layer):
    """Both mutable middleware seams must fail closed before Responses dispatch."""
    agent = _build_agent(monkeypatch)
    setattr(agent, "_disable_streaming", True)
    captured = {}
    ...
    monkeypatch.setattr("hermes_cli.middleware.apply_llm_request_middleware", _request_middleware)
